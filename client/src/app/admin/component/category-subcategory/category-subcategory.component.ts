...
export class CategorySubcategoryComponent implements OnInit {
  ...
  onSubmitcategory() {
    ...
    this.ds
      .postData('category_subcategory/addcategory', this.addcategoryform.value)
      .subscribe((res) => {
        this.data = res;
        if (this.data) {
          Swal.fire({
          ...
          Object.keys(this.addcategoryform.controls).forEach((key) => {
            this.addcategoryform.get(key)?.setErrors(null);
  ...
  onSubmitsubcategory() {
    ...
    this.ds
      .postData(
        'category_subcategory/addsubcategory',
        ...
        next: (res: any) => {
          ...
          if (res) {
            ...
            Object.keys(this.addsubcategoryform.controls).forEach((key) => {
              this.addsubcategoryform.get(key)?.setErrors(null);
              this.addsubcategoryform.get(key)?.markAsPristine();
              this.addsubcategoryform.get(key)?.markAsUntouched();
            });
          } else {
            ...
            Swal.fire({
              ...
              title: 'Error',
              text:
                res.message || 'Something went wrong while saving the data.',
            });
          }
        },
        error: (err) => {
  ...
  onSubmit(): void {
    ...
    this.ds
      .postData('category_subcategory/additem', this.additemform.value)
      .subscribe(
        (res) => {
          if (res) {
            Swal.fire({
            ...
            Object.keys(this.additemform.controls).forEach((key) => {
              this.additemform.get(key)?.setErrors(null);
              this.additemform.get(key)?.markAsPristine();
              this.additemform.get(key)?.markAsUntouched();
            });
          }
        },
        (error) => {
  ...
  onedit(item_id: any) {
    this.itemDataByid = this.allitem.find(
      (f: any) => f.item_id === parseInt(item_id)
    );
  ...
  onupdate() {
    ...
    this.ds
      .putData(
        'category_subcategory/updateallitem/' + this.data_id,
  ...
  ondelete(item_id: any) {
    console.log(item_id);
    this.allitem = this.allitem.find(
      (f: any) => f.item_id === parseInt(item_id)
    );
    this.ds
      .Delete_Data('category_subcategory/deletedByid/' + this.allitem.item_id)
