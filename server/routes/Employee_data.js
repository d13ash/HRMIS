...
const upload = multer({   //here upload is function
  ...
  router.post("/uploadfile",upload.single("Emp_Photo_Path"),(req, resp,next) => {  //here file_Path is the key for image which must be same in frotend(angular)
    const file = req.file;
    if(!file){
      return next("no file found")
    }
     const profileURL =  '/api/images/' + req.file.filename;
    resp.json({
  ...
  router.post("/uploadfiles",upload.single("Document_Path"),(req, resp,next) => {  //here file_Path is the key for image which must be same in frotend(angular)
    const file = req.file;
    if(!file){
      return next("no file found")
    }
  ...
  router.post("/uploadfilesig",upload.single("Emp_Signature_Path"),(req, resp,next) => {  //here file_Path is the key for image which must be same in frotend(angular)
    const file = req.file;
    if(!file){
      return next("no file found")
    }
...
router.get('/allempdetails', async (req, res) => {
    var query = "SELECT mbd.Emp_ID, CASE WHEN mbd.Emp_First_Name_E IS NOT NULL THEN CONCAT( mbd.Emp_First_Name_E,IF(mbd.Emp_Middle_Name_E IS NOT NULL, CONCAT(' ', mbd.Emp_Middle_Name_E), ''),IF(mbd.Emp_Last_Name_E IS NOT NULL, CONCAT(' ', mbd.Emp_Last_Name_E), '')) ELSE '' END AS FullName,mbd.Emp_Photo_Path,mbd.Father_Name_E,mbd.Mother_Name_E,mbd.Guardian_Name_E,mbd.Mobile_No,mbd.Email_Id,lt.username,lt.password FROM manpower_basic_detail mbd LEFT JOIN login_table lt ON lt.Emp_ID = mbd.Emp_ID";
...
router.get('/documentsdetail/xyz', async (req, res) => {
...
router.get('/postdata', async (req, res) => {
  var query = "SELECT Post_id, Post_name, Post_name_hindi, Display_order FROM m_post ORDER BY Display_order ASC;";
  let result = await mysql.exec(query);
  if (result.length == 0)
...
router.delete('/deletedByid/:Emp_ID',async (req,res)=>{
  var query = "DELETE mbd, lt FROM manpower_basic_detail mbd LEFT JOIN login_table lt ON lt.Emp_ID = mbd.Emp_ID WHERE mbd.Emp_ID =  ?";
  var Employee_ID = req.params.Emp_ID;
try{
  let result = await mysql.exec(query, Employee_ID)
  if(result.affectedRows < 1){ //affectRows denote any changes is done through any operation (put,post)
